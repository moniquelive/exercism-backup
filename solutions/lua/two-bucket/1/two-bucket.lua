...
local function is_forbidden(a, b, cap1, cap2, start_bucket)
	return (start_bucket == 1 and a == 0 and b == cap2)
		or (start_bucket == 2 and b == 0 and a == cap1)
end
...
return {
	measure = function(args)
		...
		if type(start_bucket) == "string" then
			start_bucket = ({ one = 1, two = 2 })[start_bucket]
		end
		if not start_bucket then error("invalid start bucket") end

		-- Quick impossibility checks (optional but nice)
		if goal > math.max(cap1, cap2) or goal % gcd(cap1, cap2) ~= 0 then
			error("impossible")
		end
		...
		while head <= #q do
			...
			for _, nxt in ipairs(neighbors(a, b, cap1, cap2)) do
				...
				if not is_forbidden(na, nb, cap1, cap2, start_bucket) then
					local k = key(na, nb)
					if not seen[k] then
