...
song :: String
song = concatMap paragraph [Fly .. Horse]
 where
  paragraph = concat . sequence [intro, swallowed, conclusion]
  ...
  swallowed' animal = printf "She swallowed the %s to catch the %s." (name (succ animal)) (descr animal)
  descr Spider = name Spider ++ " that wriggled and jiggled and tickled inside her"
  descr animal = name animal
