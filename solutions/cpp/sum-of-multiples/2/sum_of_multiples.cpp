...
auto multiple_of(const auto &factors) {
  return [&factors](int n) {
    return std::any_of(std::begin(factors), std::end(factors),
                       [n](int factor) { return n % factor == 0; });
  };
}

int to(const std::vector<int> &factors, int upper_limit) {
  auto rng = std::views::iota(0, upper_limit) |
             std::views::filter(multiple_of(factors));
