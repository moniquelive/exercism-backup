...
pub fn translate(allocator: mem.Allocator, phrase: []const u8) mem.Allocator.Error![]u8 {
    ...
    defer {
        // all the words allocated by rules() are freed by the allocator
        for (words.items) |word| allocator.free(word);
        // and the list itself is freed by the allocator
        words.deinit();
    }
    var it = mem.splitScalar(u8, phrase, ' ');
    while (it.next()) |word| try words.append(try rules(allocator, word));
